<?php
  ...
  if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['save'])){
    ...
    if (isset($_FILES['recipe_image']) && $_FILES['recipe_image']['error'] == UPLOAD_ERR_OK){
      ...
      $Rcp_Picture = 'http://15.188.49.243/RecipeSharing_WebApp/assets/dist/img/default_image.jpg'; //Default image
    }
    ...
    if(isset($ingredients) && isset($quantities) && isset($units)){
      if(empty($ingredients) || empty($quantities) || empty($units)){
        ...
        if($count_ingredients != $count_quantities || $count_quantities != $count_units){
          $ingred_error = "All ingredient details are required.";
        } else{
          $allfilled = true;
    ...
    if(empty($rcp_error) && empty($descrp_error) && empty($prep_error) && empty($cook_error) && empty($serving_error) && empty($categoryerror) && empty($ingred_error) && empty($steperror) && empty($reminderror) && empty($pic_error)){
      try{
        insertrecipe($User_ID, $Recipe_Name, $Rcp_Description, $Preparation_Time, $Cook_Time, $Servings, $Rcp_Picture, $Category_Names, $ingredients, $quantities, $units, $steps, $reminders);
        ...
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item menu-open">
            ...
            <ul class="nav nav-treeview">
              ...
              <li class="nav-item">
                <a href="http://15.188.49.243/RecipeSharing_WebApp/baker/add_recipe.php" class="nav-link active bg-gray">
                  <i class="far fa-circle nav-icon"></i>
                  <p>Add Recipe</p>
                </a>
              </li>
    ...
    <section class="content-header">
      <div class="col-sm-12">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active text-white-50">Add Recipe</li>
            </ol>
      </div>
      <div class="container-fluid">
        <div class="row">
          <div class="col-lg-auto">
            <h1><b>Add Recipe</b></h1>
            <span>Enter your recipe details to be published on the platform.</span>
          </div>
        </div>
            ...
            <div class="card">
              ...
              <form action="" method="post" enctype="multipart/form-data">
                <div class="card-body text-dark">
                 <div class="row d-grid gx-3 align-items-center">
                  <div class="col-5 form-group mb-4">
                    ...
                    <input type="text" class="form-control <?php if(!empty($rcp_error))echo 'border-danger';?>" name="recipe_name" value="<?php if(isset($Recipe_Name)) echo $Recipe_Name;?>" placeholder="Enter recipe name">
                  </div>
                  <div class="col form-group mb-4">
                    ...
                    <input type="text" class="form-control <?php if(!empty($prep_error))echo 'border-danger';?>" name="prep_time" value="<?php if(isset($Preparation_Time)) echo $Preparation_Time;?>" placeholder="Enter number">
                  </div>
                  <div class="col form-group mb-4">
                    ...
                    <input type="text" class="form-control <?php if(!empty($cook_error))echo 'border-danger';?>" name="cook_time" value="<?php if(isset($Cook_Time)) echo $Cook_Time;?>" placeholder="Enter number">
                  </div>
                  <div class="col form-group mb-4">
                    ...
                    <input type="text" class="form-control <?php if(!empty($serving_error))echo 'border-danger';?>" name="servings" value="<?php if(isset($Servings)) echo $Servings;?>" placeholder="Enter number">
                  </div>
                 </div>
                 <div class="row d-grid gx-2">
                    <div class="col-7 form-group mb-4">
                      <label >Recipe Description</label><label class="bg-danger ml-1 px-2 rounded-pill text-xs" for="inputError"><?php if(!empty($descrp_error)) echo'<i class="far fa-times-circle"></i> '.$descrp_error.'';?></label>
                        <textarea class="form-control pb-5 <?php if(!empty($descrp_error))echo 'border-danger';?>" name="description" placeholder="Enter recipe description"><?php if(isset($Rcp_Description)) echo $Rcp_Description;?></textarea>
                    </div>
                      ...
                      <div class="col form-group select2-orange">
                        ...
                        <select class="select2 mb-3" multiple="multiple" name="categories[]" data-placeholder="Select recipe category/s" data-dropdown-css-class="select2-dark" style="width: 100%;">
                          <?php foreach($result1 as $category): ?>
                            <?php $selected = in_array($category['Category_ID'], $Category_Names) ? ' selected' : '';?>
                  ...
                  <label for="customFile">Recipe Image</label><label class="bg-danger ml-1 px-2 rounded-pill text-xs" for="inputError"><?php if(!empty($pic_error)) echo'<i class="far fa-times-circle"></i> '.$pic_error.'';?></label>
                    ...
                    <img id="imagePreview" src="<?php if($Rcp_Picture !== 'http://15.188.49.243/RecipeSharing_WebApp/assets/dist/img/default_image.jpg') echo htmlspecialchars($Rcp_Picture); ?>" alt="Image Preview" style="display: <?php echo isset($Rcp_Picture) && $Rcp_Picture !== '' ? 'block' : 'none'; ?>; max-width: 200px; margin-top: 10px;">
                  </div>
                  <label class="mb-0">Recipe Ingredients</label><label class="bg-danger ml-1 px-2 rounded-pill text-xs" for="inputError"><?php if(!empty($ingred_error)) echo'<i class="far fa-times-circle"></i> '.$ingred_error.'';?></label>
                  <p class="text-sm mb-2">Add detailed components for the recipe.</p>
                  ...
                  <?php if (!empty($ingredients) || !empty($quantities) || !empty($units)): ?>
                    <?php foreach ($ingredients as $index => $ingredient): ?>
                        <div class="col-12 form-group py-0 my-0">
                          <div class="input-group my-0">
                            <input type="text" name="ingredient[]" class="form-control <?php if(empty($ingredient))echo 'border-danger';?>" value="<?php echo htmlspecialchars($ingredient); ?>" placeholder="Enter ingredient name">
                            <input type="text" name="ingredient2[]" class="form-control <?php if(empty($quantities[$index])) echo 'border-danger';?>" value="<?php echo htmlspecialchars(isset($quantities[$index]) ? $quantities[$index] : ''); ?>" placeholder="Enter ingredient quantity">
                            <input type="text" name="ingredient3[]" class="form-control <?php if(empty($units[$index])) echo 'border-danger';?> rounded-right" value="<?php echo htmlspecialchars(isset($units[$index]) ? $units[$index] : ''); ?>" placeholder="Enter ingredient unit (e.g. tsp, tbsp)">
                  ...
                  <label class="mb-0">Recipe Instructions</label><label class="bg-danger ml-1 px-2 rounded-pill text-xs" for="inputError"><?php if(!empty($steperror)) echo'<i class="far fa-times-circle"></i> '.$steperror.'';?></label>
                  <p class="text-sm mb-2">Add step-by-step instructions for making the recipe.</p>
                    ...
                    <?php foreach ($steps as $index => $step): ?>
                      <div class="col-12 form-group py-0 my-0">
                        <div class="input-group mb-2">
                          ...
                          <textarea name="step_description[]" class="form-control pl-6 rounded-right <?php if(empty($step))echo 'border-danger';?>" placeholder="Enter the instruction description"><?php echo $step; ?></textarea>
                  ...
                  <label class="mb-0">Recipe Baking Reminders</label><label class="bg-danger ml-1 px-2 rounded-pill text-xs" for="inputError"><?php if(!empty($reminderror)) echo'<i class="far fa-times-circle"></i> '.$reminderror.'';?></label>
                  <p class="text-sm mb-2">Add specific tips or suggestions to avoid baking mistakes for the recipe.</p>
                  ...
                  <?php if (!empty($reminders)): ?>
                    <?php foreach ($reminders as $index => $reminder): ?>
                      <div class="col-12 form-group py-0 my-0">
                        <div class="input-group mb-2">
                          <span class="input-group-text"><?php echo $index + 1; ?></span>
                          <textarea name="reminder_description[]" class="form-control pl-6 rounded-right <?php if(empty($reminder))echo 'border-danger';?>" placeholder="Enter the baking reminder description"><?php echo $reminder; ?></textarea>
                          <button class="btn remove-instruction" type="button"><h5><i class="bi bi-x-circle text-danger"></i></h5></button>
                        </div>
                      </div>
                    <?php endforeach; ?>
                ...
                <div class="card-footer" style= "text-align:center; background-color: #272727;">
                  <button type="submit" name="save" value="submit" class="btn bg-gradient-orange col-4 py-2 mx-1">Publish</button>
        ...
        $(document).ready(function() {
            $('#addIngredient').click(function() {
                $('#ingredientList').append(`
                    <div class="col-12 form-group py-0 my-0">
                        <div class="input-group my-0">
                        <input type="text" name="ingredient[]" class="form-control" placeholder="Enter ingredient name">
